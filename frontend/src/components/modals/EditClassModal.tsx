...
export default function EditClassModal({ open, onClose, onSuccess, classData }: EditClassModalProps) {
  const [loading, setLoading] = useState(false);
  const [selectedGrado, setSelectedGrado] = useState(classData.grado_id);
  const [selectedMateria, setSelectedMateria] = useState(classData.materia_id);
  ...
  // Establecer valores cuando se carguen los datos o cambien
  useEffect(() => {
    if (open && classData) {
      // Resetear el formulario con los datos de la clase
      reset({
      ...
      setSelectedGrado(classData.grado_id);
      setSelectedMateria(classData.materia_id);
      setValue('grado_id', classData.grado_id);
      setValue('materia_id', classData.materia_id);
    }
  }, [open, classData, reset, setValue]);
  ...
  const onSubmit = async (data: EditClassForm) => {
    try {
      setLoading(true);
      await classesApi.updateClass(classData.id, data);
  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-2xl">Editar Clase</DialogTitle>
          <DialogDescription>
            Actualiza la información de tu clase
          </DialogDescription>
        </DialogHeader>
